...
export default function ProductActions({ product }: { product: any }) {
  ...
  // Destructure 'wishlist' instead of 'wishlistIds' to match your store
  const { addToCart, toggleWishlist, wishlist } = useShopStore();

  // Check if the product exists in the wishlist array by matching IDs
  const isWish = wishlist?.some((item: any) => item.id === product.id) || false;
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <button 
          type="button"
          onClick={() => toggleWishlist(product)} // Pass the whole product object
          className={`p-4 rounded-2xl border-2 transition-all ${
            isWish ? 'bg-red-50 border-red-500' : 'bg-white border-gray-100'
          }`}
        >
